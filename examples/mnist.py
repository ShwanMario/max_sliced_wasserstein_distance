...
def main():
    ...
    parser.add_argument('--mode', type=str, choices=['max','normal'],default='max',
                        help='use max SW Distance or normal SW Distance while training')
    parser.add_argument('--mode_test', type=str, choices=['max', 'normal'], default='max',
                        help='use max SW Distance or normal SW Distance while testing')
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (x, y) in enumerate(train_loader, start=0):
            batch = trainer.train_on_batch(x,mode=mode)
            if (batch_idx + 1) % args.log_interval == 0:
                print('Train Epoch: {} ({:.2f}%) [{}/{}]\tLoss: {:.6f}'.format(
                        epoch + 1, float(epoch + 1) / (args.epochs) * 100.,
                        (batch_idx + 1), len(train_loader),
                        batch['loss'].item()),'L1_Loss:',batch['l1'].item(),'Cross_Entropy:',batch['bce'].item(),'W2Loss:',batch['w2'].item())
            train_encode.append(batch['encode'].detach())
            train_loss+=batch['real_loss'].item()
            train_targets.append(y)
        train_encode, train_targets = torch.cat(train_encode).cpu().numpy(), torch.cat(train_targets).cpu().numpy()
        ...
        if mode_test=='max':
            for test_batch_idx, (x_test, y_test) in enumerate(test_loader, start=0):
                test_evals = trainer.test_on_batch(x_test, mode=mode_test)
                test_encode.append(test_evals['encode'].detach())
                test_loss += test_evals['real_loss'].item()
                test_targets.append(y_test)
        elif mode_test=='normal':
            with torch.no_grad():
                for test_batch_idx, (x_test, y_test) in enumerate(test_loader, start=0):
                    test_evals = trainer.test_on_batch(x_test,mode=mode_test)
                    test_encode.append(test_evals['encode'].detach())
